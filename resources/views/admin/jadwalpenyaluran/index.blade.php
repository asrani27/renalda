...
@section('title', 'Jadwal Penyaluran')

@section('page-title', 'Jadwal Penyaluran')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <form action="{{ route('admin.jadwalpenyaluran.index') }}" method="GET" class="w-full sm:w-auto">
            <div class="relative">
                <input type="text" 
                       name="search" 
        ...
        <a href="{{ route('admin.jadwalpenyaluran.create') }}" 
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nama Jadwal</th>
                    ...
                    @forelse($jadwalPenyaluran as $item)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                {{ ($jadwalPenyaluran->currentpage() - 1) * $jadwalPenyaluran->perpage() + $loop->index + 1 }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-700">{{ $item->lokasi }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($item->pendamping)
                                    <div class="flex items-center gap-2">
                                        <div class="w-8 h-8 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 font-semibold text-xs">
                                            {{ strtoupper(substr($item->pendamping->nama, 0, 1)) }}
                                        </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <!-- Edit Button -->
                                    <a href="{{ route('admin.jadwalpenyaluran.edit', $item->id) }}" 
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-lg">Tidak ada data jadwal penyaluran</p>
                                    <p class="text-gray-400 text-sm mt-1">Silakan tambahkan jadwal penyaluran baru</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <!-- Pagination -->
        @if($jadwalPenyaluran->hasPages())
            <div class="px-6 py-4 border-t border-gray-200 flex justify-between items-center">
                <div class="text-sm text-gray-700">
                    Menampilkan {{ $jadwalPenyaluran->firstItem() }} sampai {{ $jadwalPenyaluran->lastItem() }} dari {{ $jadwalPenyaluran->total() }} data
                </div>
                {{ $jadwalPenyaluran->appends(['search' => request('search')])->links() }}
            </div>
        @endif
    </div>
</div>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            <div class="text-center mb-6">
                ...
                <h3 class="text-lg font-semibold text-gray-900 mb-2">Hapus Jadwal Penyaluran?</h3>
                <p class="text-gray-600">Data yang dihapus tidak dapat dikembalikan. Apakah Anda yakin ingin melanjutkan?</p>
            </div>
            <form action="{{ route('admin.jadwalpenyaluran.destroy', ':id') }}" method="POST" id="deleteForm">
                @csrf
                @method('DELETE')
                <input type="hidden" name="id" id="deleteJadwalPenyaluranId">
...
<script>
    ...
    function confirmDelete(id) {
        document.getElementById('deleteJadwalPenyaluranId').value = id;
