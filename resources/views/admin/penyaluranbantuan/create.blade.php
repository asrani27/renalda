...
@section('page-title', 'Tambah Penyaluran Bantuan')

@section('content')
<div class="mx-auto">
    <div class="bg-white rounded-xl shadow-lg p-6">
        <form action="{{ route('admin.penyaluranbantuan.store') }}" method="POST">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required
                            ...
                            <option value="dalam proses" {{ old('status') === 'dalam proses' ? 'selected' : '' }}>
                                Dalam Proses
                            </option>
                            <option value="salur" {{ old('status') === 'salur' ? 'selected' : '' }}>
                                Salur
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select id="jadwal_penyaluran_id" name="jadwal_penyaluran_id" required
                            ...
                            @foreach($jadwalPenyaluran as $jadwal)
                                <option value="{{ $jadwal->id }}" {{ old('jadwal_penyaluran_id') == $jadwal->id ? 'selected' : '' }}>
