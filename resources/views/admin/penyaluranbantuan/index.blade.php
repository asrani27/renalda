...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <form action="{{ route('admin.penyaluranbantuan.index') }}" method="GET" class="w-full sm:w-auto">
            <div class="relative">
                <input type="text" 
                       name="search" 
                       value="{{ request('search') }}" 
                       placeholder="Cari status, jadwal, atau penerima..." 
        ...
        <a href="{{ route('admin.penyaluranbantuan.create') }}" 
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Jadwal Penyaluran</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($penyaluranBantuan as $item)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                {{ ($penyaluranBantuan->currentpage() - 1) * $penyaluranBantuan->perpage() + $loop->index + 1 }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-700">{{ $item->tanggal->format('d F Y') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($item->jadwalPenyaluran)
                                    <div class="text-sm font-medium text-gray-900">{{ $item->jadwalPenyaluran->nama }}</div>
                                    <div class="text-xs text-gray-500">{{ $item->jadwalPenyaluran->lokasi }}</div>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <!-- Edit Button -->
                                    <a href="{{ route('admin.penyaluranbantuan.edit', $item->id) }}" 
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-lg">Tidak ada data penyaluran bantuan</p>
                                    <p class="text-gray-400 text-sm mt-1">Silakan tambahkan penyaluran bantuan baru</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <!-- Pagination -->
        @if($penyaluranBantuan->hasPages())
            <div class="px-6 py-4 border-t border-gray-200 flex justify-between items-center">
                <div class="text-sm text-gray-700">
                    Menampilkan {{ $penyaluranBantuan->firstItem() }} sampai {{ $penyaluranBantuan->lastItem() }} dari {{ $penyaluranBantuan->total() }} data
                </div>
                {{ $penyaluranBantuan->appends(['search' => request('search')])->links() }}
            </div>
        @endif
    </div>
</div>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            <div class="text-center mb-6">
                ...
                <h3 class="text-lg font-semibold text-gray-900 mb-2">Hapus Penyaluran Bantuan?</h3>
                <p class="text-gray-600">Data yang dihapus tidak dapat dikembalikan. Apakah Anda yakin ingin melanjutkan?</p>
            </div>
            <form action="{{ route('admin.penyaluranbantuan.destroy', ':id') }}" method="POST" id="deleteForm">
                @csrf
                @method('DELETE')
                <input type="hidden" name="id" id="deletePenyaluranBantuanId">
...
<script>
    ...
    function confirmDelete(id) {
        document.getElementById('deletePenyaluranBantuanId').value = id;
