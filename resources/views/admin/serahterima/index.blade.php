...
@section('title', 'Serah Terima Bantuan')

@section('page-title', 'Serah Terima Bantuan')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <form action="{{ route('admin.serahterima.index') }}" method="GET" class="w-full sm:w-auto">
            <div class="relative">
                <input type="text" 
                       name="search" 
                       value="{{ request('search') }}" 
                       placeholder="Cari nomor BAST, penerima, atau pendamping..." 
        ...
        <a href="{{ route('admin.serahterima.create') }}" 
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nomor BAST</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tanggal BAST</th>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Foto BAST</th>
                    ...
                    @forelse($serahTerima as $item)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                {{ ($serahTerima->currentpage() - 1) * $serahTerima->perpage() + $loop->index + 1 }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $item->nomor_bast }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-700">{{ $item->tanggal_bast->format('d F Y') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($item->foto_bast)
                                    <a href="{{ asset('uploads/serah_terima/' . $item->foto_bast) }}" 
                                       target="_blank"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <!-- Edit Button -->
                                    <a href="{{ route('admin.serahterima.edit', $item->id) }}" 
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-lg">Tidak ada data serah terima bantuan</p>
                                    <p class="text-gray-400 text-sm mt-1">Silakan tambahkan serah terima bantuan baru</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <!-- Pagination -->
        @if($serahTerima->hasPages())
            <div class="px-6 py-4 border-t border-gray-200 flex justify-between items-center">
                <div class="text-sm text-gray-700">
                    Menampilkan {{ $serahTerima->firstItem() }} sampai {{ $serahTerima->lastItem() }} dari {{ $serahTerima->total() }} data
                </div>
                {{ $serahTerima->appends(['search' => request('search')])->links() }}
            </div>
        @endif
    </div>
</div>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            <div class="text-center mb-6">
                ...
                <h3 class="text-lg font-semibold text-gray-900 mb-2">Hapus Serah Terima Bantuan?</h3>
                <p class="text-gray-600">Data yang dihapus tidak dapat dikembalikan. Apakah Anda yakin ingin melanjutkan?</p>
            </div>
            <form action="{{ route('admin.serahterima.destroy', ':id') }}" method="POST" id="deleteForm">
                @csrf
                @method('DELETE')
                <input type="hidden" name="id" id="deleteSerahTerimaId">
...
<script>
    ...
    function confirmDelete(id) {
        document.getElementById('deleteSerahTerimaId').value = id;
