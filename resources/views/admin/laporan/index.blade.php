        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
            <div class="p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="bg-emerald-100 text-emerald-800 text-xs font-medium px-2.5 py-0.5 rounded">Populer</span>
                </div>
...
<div id="modalSuratPernyataan" class="fixed inset-0 bg-black bg-opacity-50 z-50 hidden flex items-center justify-center">
    <div class="bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 transform transition-all">
        <div class="bg-gradient-to-r from-purple-500 to-purple-600 px-6 py-4 rounded-t-lg">
            <div class="flex items-center justify-between">
                ...
                <button onclick="document.getElementById('modalSuratPernyataan').classList.add('hidden')" class="text-white hover:text-gray-200">
        ...
        <div class="p-6">
            <form action="{{ route('admin.laporan.surat-pernyataan') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="penerima_id" id="penerima_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm">
                        <option value="">-- Pilih Penerima --</option>
                        @foreach(\App\Models\Penerima::with('calonPenerima')->get() as $penerima)
                            <option value="{{ $penerima->id }}">{{ $penerima->calonPenerima->nama ?? 'N/A' }} - {{ $penerima->calonPenerima->nik ?? 'N/A' }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex gap-3 pt-2">
                    <button type="button" onclick="document.getElementById('modalSuratPernyataan').classList.add('hidden')" 
